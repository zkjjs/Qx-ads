...
[rewrite]
^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus url script-request-header https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/WeTalk_Loon.js

[script]
cron "20 8,20 * * *" script-path=https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/WeTalk_Loon.js,tag=WeTalk签到
...
[Script]
http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=WeTalk_Loon.js,requires-body=false,timeout=10,tag=WeTalk抓包

cron "20 8,20 * * *" script-path=WeTalk_Loon.js,timeout=120,tag=WeTalk签到
...
// ─── HTTP 请求（Loon 用 $httpClient，封装为 Promise） ─────────────────────────
...
function runAccount(acc, index, total) {
  ...
  function fetchApi(path) {
    return fetchGet(buildUrl(path, acc.capture), headers);
  }
